...
describe("NftCollection", function () {
  ...
  beforeEach(async function () {
    ...
    nft = await NFT.deploy("MyNFT", "MNFT", 10, "https://mybaseuri.com/");
  ...
  it("Should allow only owner to mint", async function () {
    await expect(nft.connect(addr1).safeMint(addr1.address, 1))
  ...
  it("Should fail when mint exceeds maxSupply", async function () {
    ...
    await expect(nft.safeMint(owner.address, 11))
